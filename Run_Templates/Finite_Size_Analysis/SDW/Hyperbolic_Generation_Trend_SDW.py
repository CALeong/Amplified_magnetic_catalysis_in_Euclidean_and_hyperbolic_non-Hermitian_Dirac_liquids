...
def magnetic_order_parameters_localgen(rawdata, p, num_gens):
...
result_large = magnetic_order_parameters_localgen(reldata_large, pval, nval_large)[0]

rawdata_small = np.load(rawdir_smaller)
reldata_small = rawdata_small[np.where(np.abs(np.real(rawdata_small[:, 0] - uval)) < 0.001)[0], :]
result_small = magnetic_order_parameters_localgen(reldata_small, pval, nval_small)[0]
